...
# 浏览器初始化
def driver_init():
    driver.get("http://localhost/ecshop/")  # 打开ecshop商城首页
    driver.maximize_window()  # 浏览器窗口最大化
    time.sleep(1)
    driver.find_element_by_xpath("//*[@id='ECS_MEMBERZONE']/a[2]/img").click()  # 通过xpath方法定位到“注册”按钮，注意里面的值要把双引号换成单引号
    time.sleep(2)


# 根据对网页的分析我们把获取文本框的路径进行封装,封装获取name的方法 定义name变量
...
# 编写主程序,调用封装的方法来实现注册
def run_main():
    user_name_info = get_userinfo()
    email = user_name_info + "@163.com"
    user_contact_info = get_contactinfo()
    driver_init()
